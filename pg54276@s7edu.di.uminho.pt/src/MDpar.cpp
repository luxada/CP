...
double PotentialEcomputeAccelerations();
...
int main()
{
    ...
    for (i=0; i<NumTime+1; i++) {
        ...
        double* array;
        array = VelocityVerlet(dt, i+1, tfp);
        Press = array[1]*PressFac;
        //  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
double PotentialEcomputeAccelerations(){
    ...
    #pragma omp parallel for reduction(+:Pot) reduction(+:transpostaA[:3][:N]) schedule(dynamic,50)
    for (i=0; i<N; i++) {
        ...
        for (j=i+1; j<N; j++){
            ...
            r2 += (g-transpostaR[0][j])*(g-transpostaR[0][j]);
            r2 += (h-transpostaR[1][j])*(h-transpostaR[1][j]);
            r2 += (e-transpostaR[2][j])*(e-transpostaR[2][j]);
...
double* VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    resultado[0] = PotentialEcomputeAccelerations();
