...
void transpose_back(double A[][3], double B[3][N]); 
...
double Potential(){
    ...
    double transposed[3][N];
    transpose(r, transposed);
    ...
    for (i=0; i<N; i++) {
        ...
        d = transposed[2][i];
            ...
            for (j=i+1; j<N; j++){
                ...
                r2 += (b-transposed[0][j])*(b-transposed[0][j]);
                r2 += (c-transposed[1][j])*(c-transposed[1][j]);
                r2 += (d-transposed[2][j])*(d-transposed[2][j]);


                Pot += contaPotencial(r2);//4.*(term1 - term2);
            }
    }
    return 2*Pot;
}
...
double contaComputeAccelerations(double rSqd){
    ...
    //f = 24 * (2 * 1/(help*help*help*rSqd) - (1 / (help*help))); // mudei
...
void computeAccelerations() {
    ...
    transpose_back(a, transpostaA);
    transpose_back(r,transpostaR);
}
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (i=0; i<N; i++) {
        ...
        transpostaR[0][i] += transpostaV[0][i]*dt + 0.5*transpostaA[0][i]*dt*dt;
        transpostaV[0][i] += 0.5*transpostaA[0][i]*dt;

        transpostaR[1][i] += transpostaV[1][i]*dt + 0.5*transpostaA[1][i]*dt*dt;
        transpostaV[1][i] += 0.5*transpostaA[1][i]*dt;
        
        transpostaR[2][i] += transpostaV[2][i]*dt + 0.5*transpostaA[2][i]*dt*dt;
        transpostaV[2][i] += 0.5*transpostaA[2][i]*dt;
    ...
    for (i=0; i<N; i++) {
        ...
        if (transpostaR[0][i]<0. || transpostaR[0][i]>=L) {
            transpostaV[0][i] *=-1.; //- elastic walls
            psum += 2*m*fabs(transpostaV[0][i])/dt;  // contribution to pressure from "left" walls
        }

        if (transpostaR[1][i]<0. || transpostaR[1][i]>=L) {
            transpostaV[1][i] *=-1.; //- elastic walls
            psum += 2*m*fabs(transpostaV[1][i])/dt;  // contribution to pressure from "left" walls
        }

        if (transpostaR[2][i]<0. || transpostaR[2][i]>=L) {
            transpostaV[2][i] *=-1.; //- elastic walls
            psum += 2*m*fabs(transpostaV[2][i])/dt;  // contribution to pressure from "left" walls
        }
    }
