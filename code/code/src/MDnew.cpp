...
//
// loop unroll do ciclo interior (j<3)
...
//
// usa sqrt e pow e loop unroll  => unico custo é a conta final
double Potential() {
    double quot, r2, rnorm, term1, term2, Pot, result;
    int i, j;
    
    double transposed[3][N];

    //void transposeMatrix(double original[][M], double transposed[][N], int N, int M) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < 3; j++) {
    ...
    for (i=0; i<N; i++) {
        for (j=0; j<N; j++) {
            ...
            if (j!=i) {
                r2=0.;
                //for (k=0; k<3; k++) {
                    r2 += (transposed[0][i]-transposed[0][j])*(transposed[0][i]-transposed[0][j]);

                    r2 += (transposed[1][i]-transposed[1][j])*(transposed[1][i]-transposed[1][j]);

                    r2 += (transposed[2][i]-transposed[2][j])*(transposed[2][i]-transposed[2][j]);
                //}
                //rnorm=sqrt(r2);
                quot=(sigma*sigma)/r2;
                //term1 = pow(quot,12.);
...
void computeAccelerations() {
    ...
    double transposedd[3][N];

    //void transposeMatrix(double original[][M], double transposed[][N], int N, int M) {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < N ; j++) {
            transposedd[i][j] = r[j][i];
        }
    }
    ...
    for (i = 0; i < N-1; i++) {   // loop over all distinct pairs i,j
        for (j = i+1; j < N; j++) {
            ...
            //for (k = 0; k < 3; k++) {
                ...
                rij[k] = transposedd[0][i] - r[0][j];
                //  sum of squares of the components
                rSqd += rij[0] * rij[0];

                //  component-by-componenent position of i relative to j
                rij[1] = transposedd[1][i] - transposedd[1][j];
                //  sum of squares of the components
                rSqd += rij[1] * rij[1];

                //  component-by-componenent position of i relative to j
                rij[2] = transposedd[2][i] - transposedd[2][j];
                //  sum of squares of the components
                rSqd += rij[2] * rij[2];
            //}
            ...
            f = 24 * ((2 * (1 / (help*help*help*rSqd)))    /*(2 * pow(rSqd, -7)*/ - (1 / (help*help)));//    pow(rSqd, -4));
